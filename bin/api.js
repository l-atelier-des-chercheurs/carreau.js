...
var api = (function() {
  const API = {
    ...
    findFirstFilenameNotTaken: (confPath, fileName) => {
      return findFirstFilenameNotTaken(confPath, fileName);
    },
    getMetaFileOfConf: slugConfName => {
      return getMetaFileOfConf(slugConfName);
    },
    readConfMeta: slugConfName => {
      return readConfMeta(slugConfName);
    },
    getCurrentDate: (format = settings.metaDateFormat) => {
      return getCurrentDate(format);
    },
    textifyObj: obj => {
      return textifyObj(obj);
    },
    storeData: (mpath, d, e) => {
      return storeData(mpath, d, e);
    },
    parseData: d => {
      return parseData(d);
    }
  ...
  function findFirstFilenameNotTaken(confPath, fileName) {
    return new Promise(function(resolve, reject) {
      ...
      dev.logverbose(
        'Looking for existing file with name : ' +
      ...
      try {
        ...
        while (
          ...
          dev.logverbose(
            '- - following path is already taken : newPathToFile = ' +
              newPathToFile +
              ' or newPathToMeta = ' +
              newPathToMeta
          );
          ...
          newMetaFileName =
            fileNameWithoutExtension + '-' + index + settings.metaFileext;
  ...
  function getMetaFileOfConf(slugConfName) {
    ...
    let metaPath = path.join(
      confPath,
      settings.confMetafilename + settings.metaFileext
    );
    return metaPath;
  }
  ...
  function readConfMeta(slugConfName) {
    return new Promise(function(resolve, reject) {
      ...
      if (folderMetadata.introduction !== undefined) {
        try {
          folderMetadata.introduction = mm.parse(
            folderMetadata.introduction
          ).content;
        } catch (err) {
          console.log(
            'Couldn’t parse conf introduction for conf ' + slugConfName
          );
        }
      }
  ...
  function textifyObj(obj) {
    ...
    for (var prop in obj) {
      ...
      if (typeof value === 'array' || typeof value === 'object') {
        ...
        dev.logverbose(
          '2. WARNING : found a delimiter in string, replacing it with a backslash'
        );
